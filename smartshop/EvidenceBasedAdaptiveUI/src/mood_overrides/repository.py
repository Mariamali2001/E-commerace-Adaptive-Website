"""Discover mood-level UI overrides that differ from survey defaults.

For each mood this module compares UI preferences to the default repository
and keeps an override only when the mood majority differs from the default
and is supported by multi-source evidence (statistics, Random Forest, SHAP,
association rules). Default values are never duplicated.
"""
...
from src.persona_overrides.repository import load_defaults
from src.preprocessing.columns import ALL_UI_COLUMNS

logger = logging.getLogger(__name__)

MOOD_COLUMN = "current_mood"
PREDICTOR = "current_mood"
...
def _rule_matches_mood(antecedent: str, mood: str) -> bool:
    """True when the rule antecedent is mood-only for this mood."""
    parts = [part.strip() for part in str(antecedent).split(",") if part.strip()]
    if len(parts) != 1:
        return False
    item = parts[0]
    if not item.startswith("Mood="):
        return False
    return item.split("=", maxsplit=1)[1].strip() == mood
...
def _lookup_association(
    ...
    """Best mood-only association rule matching this UI value."""
    if rules.empty:
        return None

    best: dict[str, float] | None = None
    for _, row in rules.iterrows():
        if not _rule_matches_mood(str(row.get("Antecedent", "")), mood):
            continue
...
def mood_majority(series: pd.Series) -> tuple[str | None, int, float]:
    """Return majority value, count, and share within a mood group."""
    valid = series.dropna()
    total = int(len(valid))
    if total == 0:
        return None, 0, 0.0
...
def build_mood_overrides(
    ...
    """Build mood override repositories and a flat export table."""
    n_total = len(df)
    repositories: list[dict[str, Any]] = []
    flat_rows: list[dict[str, Any]] = []

    moods = sorted(df[MOOD_COLUMN].dropna().unique())

    for mood in moods:
        group = df[df[MOOD_COLUMN] == mood]
        ...
        for ui_element in ALL_UI_COLUMNS:
            ...
            majority_value, count, share = mood_majority(group[ui_element])
            if majority_value is None:
                continue
            if count < MIN_MOOD_COUNT or share < MIN_MOOD_SHARE:
                continue
            ...
            rule_metrics = _lookup_association(rules, mood, ui_element, majority_value)
...
@dataclass
class MoodOverrideResult:
    """Outputs from the mood override pipeline."""
...
def run_mood_override_pipeline(
    project_root: Path,
    output_dir: Path,
    reports_dir: Path,
) -> MoodOverrideResult:
    """Load inputs, discover evidence-backed mood overrides, and export them."""
    ...
    repositories, table = build_mood_overrides(
        df, defaults, statistical, importance, shap, rules
    )

    export_paths: dict[str, Path] = {}

    json_path = output_dir / "mood_overrides.json"
    json_path.write_text(
        json.dumps(repositories, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    export_paths["mood_overrides_json"] = json_path

    xlsx_path = reports_dir / "mood_overrides.xlsx"
    table.to_excel(xlsx_path, index=False)
    export_paths["mood_overrides_xlsx"] = xlsx_path

    csv_path = reports_dir / "mood_overrides.csv"
    table.to_csv(csv_path, index=False)
    export_paths["mood_overrides_csv"] = csv_path

    overrides_per_mood = {item["mood"]: item["n_overrides"] for item in repositories}
    avg_confidence = float(table["Confidence"].mean()) if not table.empty else 0.0

    summary = {
        "overrides_per_mood": overrides_per_mood,
        "total_overrides": int(len(table)),
        "average_confidence": round(avg_confidence, 6),
        "moods": len(repositories),
    }

    summary_md = reports_dir / "mood_overrides_summary.md"
    lines = [
        "# Mood Overrides Summary",
        "",
        "Only UI elements that differ from defaults and have multi-source evidence.",
        "",
        f"- Moods: {summary['moods']}",
        f"- Total overrides: {summary['total_overrides']}",
        f"- Average confidence: {summary['average_confidence']:.4f}",
        "",
        "## Overrides per mood",
    ]
    for mood, count in overrides_per_mood.items():
        lines.append(f"- {mood}: {count}")
    summary_md.write_text("\n".join(lines), encoding="utf-8")
    export_paths["summary_md"] = summary_md

    logger.info(
        "Mood overrides: %s total across %s moods (avg confidence=%.3f)",
        summary["total_overrides"],
        summary["moods"],
        summary["average_confidence"],
    )
    return MoodOverrideResult(
